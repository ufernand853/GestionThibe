...
router.post(
  ...
  asyncHandler(async (req, res) => {
    const { username, email, password, roleId, status, localSaleEnabled, localSaleLocationId } = req.body || {};
    ...
    const saleLocation = localSaleEnabled ? await Location.findById(localSaleLocationId) : null;
    if (localSaleEnabled && (!saleLocation || !saleLocation.isLocal || saleLocation.status !== 'active')) {
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (localSaleEnabled !== undefined || localSaleLocationId !== undefined) {
      ...
      const saleLocation = enabled ? await Location.findById(locationId) : null;
      if (enabled && (!saleLocation || !saleLocation.isLocal || saleLocation.status !== 'active')) {
        throw new HttpError(400, 'Debe seleccionar un local activo para habilitar Venta desde Local');
      }
